    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            ...
            <h2 className="text-2xl font-black text-gray-900">{income.toFixed(3)} {t.currency}</h2>
         </div>
         <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-4">
            ...
            <h2 className="text-2xl font-black text-gray-900">{expense.toFixed(3)} {t.currency}</h2>
         </div>
         <div className="bg-[#100F19] p-8 rounded-[2.5rem] shadow-xl text-white">
            <div className="flex items-center gap-4 mb-4">
               ...
               <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t.netProfit}</span>
            </div>
            <h2 className="text-2xl font-black">{(income - expense).toFixed(3)} {t.currency}</h2>
         </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-[#483383] text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg"
        >
          <Plus size={20} />
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-start">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.type}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.categories}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.amount}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.date}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.note}</th>
            </tr>
          </thead>
            ...
            {dbData.accounting.map((entry) => (
              <tr key={entry.id} className="hover:bg-gray-50/50">
                <td className="px-6 py-4">
                  <div className={`flex items-center gap-2 font-bold text-xs ${entry.type === 'income' ? 'text-green-600' : 'text-red-500'}`}>
                    {entry.type === 'income' ? <ArrowUpCircle size={16} /> : <ArrowDownCircle size={16} />}
                    {entry.type === 'income' ? t.income : t.expense}
                  </div>
                </td>
                <td className="px-6 py-4 font-bold text-gray-900">{entry.category}</td>
                <td className="px-6 py-4 font-bold text-gray-900">{entry.amount.toFixed(3)} {t.currency}</td>
            ...
            <div className="p-8 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setForm({...form, type: 'income'})} className={`flex-1 py-3 rounded-xl font-bold border transition-all ${form.type === 'income' ? 'bg-green-50 border-green-200 text-green-600' : 'border-gray-100 text-gray-400'}`}>{t.income}</button>
                  <button onClick={() => setForm({...form, type: 'expense'})} className={`flex-1 py-3 rounded-xl font-bold border transition-all ${form.type === 'expense' ? 'bg-red-50 border-red-200 text-red-600' : 'border-gray-100 text-gray-400'}`}>{t.expense}</button>
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t.categories}</label>
