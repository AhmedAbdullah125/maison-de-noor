      ...
      <div className="flex-1 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-x-auto no-scrollbar flex-1">
          <table className="w-full text-start border-collapse">
            <thead className="sticky top-0 bg-gray-50 border-b border-gray-100 z-10">
              <tr>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start whitespace-nowrap">{t.time}</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start whitespace-nowrap">{t.actor}</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start whitespace-nowrap">{t.action}</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start whitespace-nowrap">{t.entity}</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start whitespace-nowrap">{t.details}</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start whitespace-nowrap">{t.severity}</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-6 py-4">
                       <div className="flex flex-col">
                          <span className="text-xs font-bold text-gray-900 whitespace-nowrap" dir="ltr">{ts.date}</span>
                    ...
                    <td className="px-6 py-4">
                       <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                             <span className="text-xs font-bold text-gray-900 whitespace-nowrap">{log.actorName}</span>
                             <span className="text-[10px] text-gray-400 uppercase font-black">{t[log.actorType as keyof typeof t] || log.actorType}</span>
                          </div>
                       </div>
                    </td>
                    <td className="px-6 py-4">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium text-gray-700 whitespace-nowrap capitalize">{log.actionType.replace('-', ' ')}</span>
                       </div>
                    </td>
                    <td className="px-6 py-4">
                       <div className="flex flex-col">
                          <span className="text-xs font-bold text-[#483383] whitespace-nowrap">{log.entityType}</span>
                    ...
                    <td className="px-6 py-4">
                       <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[10px] font-bold ${getSeverityStyles(log.severity)}`}>
          ...
          {paginatedLogs.length === 0 && (
            <div className="py-32 flex flex-col items-center justify-center text-center">
               ...
               <p className="text-gray-400 font-bold">{t.noRecentLogs}</p>
            </div>
          )}
        </div>
        ...
        <div className="px-8 py-4 bg-gray-50 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4">
           <div className="flex items-center gap-4">
              <span className="text-[11px] text-gray-400 font-bold uppercase tracking-wider">{t.rowsPerPage}</span>
              <select 
                className="bg-white border border-gray-200 rounded-lg text-xs font-bold p-1"
                value={rowsPerPage}
           ...
           <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black text-gray-900 px-4 min-w-[100px] text-center">
        ...
        <div className="fixed inset-0 z-[110] flex justify-end animate-fadeIn bg-black/20 backdrop-blur-[2px]">
           ...
           >
              <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between shrink-0">
                 <h3 className="text-xl font-black text-gray-900">{t.logDetails}</h3>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar p-8 space-y-8">
                 <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                    <div className="flex items-center gap-4 mb-4">
                       ...
                       <div>
                          <h4 className="text-lg font-black text-gray-900">{selectedLog.shortTitle}</h4>
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest" dir="ltr">
                             {selectedLog.id}
                          </span>
                       </div>
                    </div>
                 ...
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                       ...
                       <div className="flex flex-col" dir="ltr">
                          <span className="text-sm font-black text-gray-900">{formatTimestamp(selectedLog.timestamp).date}</span>
                          <span className="text-xs text-gray-500">{formatTimestamp(selectedLog.timestamp).time}</span>
                       </div>
                    </div>
                    <div className="space-y-1">
                       <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t.actor}</span>
                       <div className="flex flex-col">
                          <span className="text-sm font-black text-gray-900">{selectedLog.actorName}</span>
                          <span className="text-[10px] text-gray-500 uppercase font-bold">{t[selectedLog.actorType as keyof typeof t] || selectedLog.actorType}</span>
                       </div>
                    </div>
                    <div className="space-y-1">
                       <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t.entity}</span>
                       <div className="flex flex-col">
                          <span className="text-sm font-black text-[#483383]">{selectedLog.entityType}</span>
                          <span className="text-xs text-gray-500">{selectedLog.entityName || '-'}</span>
                       </div>
                    </div>
                    <div className="space-y-1">
                       <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t.action}</span>
                       <div className="flex flex-col">
                          <span className="text-sm font-black text-gray-900 capitalize">{selectedLog.actionType.replace('-', ' ')}</span>
                          <span className="text-[10px] text-gray-500 uppercase font-bold">Operation Code</span>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="pt-8 border-t border-gray-50 space-y-4">
                    <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                       ...
                       <div className="flex flex-col">
                          <span className="text-[10px] font-bold text-gray-400 uppercase">{t.ip}</span>
                          <span className="text-xs font-black text-gray-900" dir="ltr">{selectedLog.ip}</span>
                       </div>
                    </div>
                    <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                       <Monitor size={20} className="text-gray-400" />
                       <div className="flex flex-col">
                          <span className="text-[10px] font-bold text-gray-400 uppercase">{t.device}</span>
                          <span className="text-xs font-black text-gray-900 leading-tight" dir="ltr">{selectedLog.device}</span>
                       </div>
                    </div>
                 </div>
              </div>
              ...
              <div className="p-8 border-t border-gray-100 shrink-0">
                 <button 
                   ...
                   className="w-full py-4 bg-gray-100 text-gray-900 font-black rounded-2xl hover:bg-gray-200 transition-colors"
                 >
                    {t.close}
                 </button>
              </div>
