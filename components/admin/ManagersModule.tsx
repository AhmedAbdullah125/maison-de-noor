    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-1 gap-3">
          ...
          <select 
            className="bg-white border border-gray-200 px-4 py-3 rounded-2xl text-xs font-bold outline-none"
            value={statusFilter}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-start">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.fullName}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.username}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.email}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.status}</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-start">{t.permissions}</th>
            ...
            {filtered.map((mgr) => {
              ...
              return (
                <tr key={mgr.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4 font-bold text-gray-900">{mgr.fullName}</td>
                  ...
                  <td className="px-6 py-4">
                     <button onClick={() => handleStatusToggle(mgr)} className="flex items-center">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border flex items-center gap-1.5 ${mgr.status === 'active' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                  ...
                  <td className="px-6 py-4">
                     <span className={`text-[10px] font-bold ${isSuper ? 'text-[#483383]' : 'text-gray-400'}`}>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden animate-scaleIn flex flex-col max-h-[90vh]">
            <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center shrink-0">
              <h3 className="text-xl font-bold">{editingManager ? t.editManager : t.addManager}</h3>
            ...
            <div className="p-8 space-y-8 overflow-y-auto no-scrollbar">
              ...
              <div>
                 <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-base font-bold text-gray-900">{t.permissions}</h4>
                 </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {permissionKeys.map(key => (
                       <div key={key} onClick={() => handleTogglePermission(key)} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors">
                          <span className="text-xs font-bold text-gray-600 capitalize">{t[key as keyof typeof t] || key.replace(/([A-Z])/g, ' $1')}</span>
