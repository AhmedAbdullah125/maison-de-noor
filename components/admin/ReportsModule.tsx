    ...
    <div className="space-y-8 animate-fadeIn pb-20">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-gray-900">{t.salesReports}</h2>
            <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">{filteredBookings.length} {t.completed}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${filterType === 'current_month' ? 'bg-[#483383] text-white' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
          >
            {t.currentMonth}
          </button>
          <button 
            onClick={() => setFilterType('today')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${filterType === 'today' ? 'bg-[#483383] text-white' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
          >
            {t.today}
          </button>
          ...
          <button 
            onClick={exportToCSV}
            className="flex items-center gap-2 bg-green-50 text-green-600 px-4 py-2 rounded-xl text-xs font-bold hover:bg-green-100 transition-all ml-2"
          >
            <Download size={16} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
            <div className="flex justify-between items-start mb-4">
               ...
               <span className="flex items-center text-green-500 text-[10px] font-black bg-green-50 px-2 py-0.5 rounded-lg">
                  <ArrowUpRight size={10} /> 12%
               </span>
            </div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.totalRevenue}</p>
            <h3 className="text-2xl font-black text-gray-900">{stats.revenue.toFixed(3)} <span className="text-xs font-bold text-gray-400">{t.currency}</span></h3>
         </div>
         ...
         <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.totalCompletedBookings}</p>
         ...
         <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.uniqueClients}</p>
            <h3 className="text-2xl font-black text-gray-900">{stats.clients}</h3>
         </div>
         ...
         <div className="bg-[#100F19] p-6 rounded-[2rem] shadow-xl text-white">
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t.averageOrderValue}</p>
            <h3 className="text-2xl font-black">{stats.aov.toFixed(3)} <span className="text-xs font-bold text-gray-500">{t.currency}</span></h3>
         </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
               <h3 className="text-lg font-black text-gray-900 mb-8 flex items-center justify-between">
                  {t.dailyRevenueTrend}
                  <span className="text-[10px] text-gray-400 uppercase font-bold">{customRange.from} - {customRange.to}</span>
               </h3>
               
               <div className="relative h-64 flex items-end gap-2 px-2 overflow-x-auto no-scrollbar">
                  {dailyChartData.length === 0 ? (
                     <div className="absolute inset-0 flex items-center justify-center text-gray-300 text-sm font-bold uppercase italic">No data available</div>
                  ) : (
                    dailyChartData.map((d, i) => (
                      <div key={i} className="flex-1 min-w-[30px] group relative">
                         ...
                         />
                         <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#100F19] text-white text-[9px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20">
                            {d.revenue.toFixed(2)}
                         </div>
                         <div className="text-[8px] text-gray-400 font-bold mt-2 rotate-45 origin-left whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">
                            {d.date.slice(5)}
                         </div>
                      </div>
                    ))
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
               <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                  <h3 className="text-lg font-black text-gray-900">{t.topSellingServices}</h3>
                  <button className="text-xs font-bold text-[#483383] hover:underline flex items-center gap-1">{t.viewAll} <ChevronRight size={14} /></button>
               </div>
               <table className="w-full text-start">
                  <thead className="bg-gray-50/50">
                     <tr>
                        <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase text-start">{t.service}</th>
                        <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase text-start">{t.categories}</th>
                        <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase text-center">{t.salesCount}</th>
                        <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase text-end">{t.totalRevenue}</th>
                     </tr>
                  </thead>
                     ...
                     {topServices.map((s, i) => (
                        <tr key={i} className="hover:bg-gray-50 transition-colors">
                           ...
                           <td className="px-8 py-4 text-end">
                              <span className="text-xs font-black text-[#483383]">{s.revenue.toFixed(3)}</span>
                           </td>
                        </tr>
                     ))}
                  </tbody>
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
               <div className="p-8 border-b border-gray-50">
                  <h3 className="text-lg font-black text-gray-900">{t.mostRequestedCategories}</h3>
               </div>
                     ...
                     return (
                        <div key={i} className="space-y-2">
                           <div className="flex justify-between items-end">
                              ...
                              <span className="text-[10px] font-bold text-gray-400">{c.revenue.toFixed(2)} {t.currency}</span>
                           </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
               <div className="p-8 border-b border-gray-50">
                  <h3 className="text-lg font-black text-gray-900">{t.mostActiveClients}</h3>
               </div>
                     ...
                     <div key={i} className="px-8 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors cursor-default">
                        <div className="flex items-center gap-3">
                           ...
                           <div className="flex flex-col">
                              ...
                              <span className="text-[9px] font-bold text-gray-400 uppercase">{u.count} {t.bookings}</span>
                           </div>
                        </div>
