    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-gray-900">{t.serviceAddons}</h2>
            <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">{addons.length} Add-on Categories</p>
          </div>
        </div>

        <button 
          onClick={() => handleOpenModal()}
          className="bg-[#483383] text-white px-6 py-4 rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-[#483383]/20 active:scale-95 transition-all"
        >
          <Plus size={20} />
        ...
        {addons.map((addon) => (
          <div key={addon.id} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col group">
             <div className="flex justify-between items-start mb-6">
                <div>
                   <h3 className="text-lg font-black text-gray-900">{lang === 'ar' ? addon.titleAr : addon.titleEn}</h3>
                   <div className="flex flex-wrap gap-2 mt-2">
                      <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-lg border ${addon.required ? 'bg-red-50 text-red-500 border-red-100' : 'bg-green-50 text-green-600 border-green-100'}`}>
                         {addon.required ? t.required : t.optional}
                      </span>
                      <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded-lg bg-violet-50 text-[#483383] border border-violet-100">
             ...
             <div className="space-y-2 mb-8">
                {addon.items.slice(0, 3).map((item, idx) => (
                   <div key={idx} className="flex justify-between items-center text-xs">
                      <span className="text-gray-500 font-medium">{lang === 'ar' ? item.labelAr : item.labelEn}</span>
                      <span className="text-gray-900 font-bold">{item.price.toFixed(3)} {t.currency}</span>
                   </div>
                ))}
                {addon.items.length > 3 && (
                   <div className="text-[10px] text-gray-400 font-bold text-center mt-2">+{addon.items.length - 3} More Options</div>
                )}
             </div>

             <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between">
                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{addon.items.length} {t.items}</span>
                <button onClick={() => handleOpenModal(addon)} className="text-xs font-black text-[#483383] hover:underline flex items-center gap-1">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-3xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-scaleIn flex flex-col max-h-[90vh]">
             <div className="px-10 py-8 border-b border-gray-100 flex justify-between items-center shrink-0">
                <h3 className="text-2xl font-black text-gray-900">{editingAddon ? t.editAddon : t.addAddon}</h3>
             ...
             <div className="p-10 overflow-y-auto no-scrollbar space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-2">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest">{t.titleEn}</label>
                      <input 
                        type="text" 
                   ...
                   <div className="space-y-2 text-right">
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest">{t.titleAr}</label>
                      <input 
                        type="text" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-1">{t.required}</label>
                        <div className="flex gap-2 p-1 bg-gray-50 rounded-2xl w-fit">
                            <button 
                                onClick={() => setForm({...form, required: true})}
                                className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all ${form.required ? 'bg-[#483383] text-white' : 'text-gray-400'}`}
                            >
                                {t.required}
                            </button>
                            <button 
                                onClick={() => setForm({...form, required: false})}
                                className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all ${!form.required ? 'bg-white shadow-sm text-gray-900 border border-gray-100' : 'text-gray-400'}`}
                            >
                                {t.optional}
                            </button>
                        </div>
                    ...
                    <div className="space-y-3">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-1">{t.selectionType}</label>
                        <div className="flex gap-2 p-1 bg-gray-50 rounded-2xl w-fit">
                            <button 
                                onClick={() => setForm({...form, selectionType: 'single'})}
                                className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all ${form.selectionType === 'single' ? 'bg-[#483383] text-white' : 'text-gray-400'}`}
                            >
                                {t.singleChoice}
                            </button>
                            <button 
                                onClick={() => setForm({...form, selectionType: 'multiple'})}
                                className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all ${form.selectionType === 'multiple' ? 'bg-[#483383] text-white' : 'text-gray-400'}`}
                            >
                                {t.multipleChoice}
                            </button>
                        </div>
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between">
                        ...
                        <button 
                            ...
                            className="text-xs font-black text-[#483383] flex items-center gap-1 hover:underline"
                        >
                        ...
                        {form.items?.map((item, idx) => (
                           <div key={item.id} className="flex flex-col md:flex-row gap-4 p-5 bg-gray-50 rounded-2xl border border-gray-100 group">
                               <div className="flex-1 space-y-1">
                                  <label className="text-[10px] font-black text-gray-400 uppercase">{t.labelEn}</label>
                                  <input 
                                    type="text" 
                               ...
                               <div className="flex-1 space-y-1 text-right">
                                  <label className="text-[10px] font-black text-gray-400 uppercase">{t.labelAr}</label>
                                  <input 
                                    type="text" 
                               ...
                               <div className="w-full md:w-32 space-y-1">
                                  <label className="text-[10px] font-black text-gray-400 uppercase">{t.price} (KWD)</label>
                                  <input 
                                    type="number" 
                                    step="0.001"
                                    className="w-full bg-white border border-gray-100 rounded-xl p-3 text-xs outline-none focus:border-[#483383]"
             ...
             <div className="px-10 py-8 border-t border-gray-100 flex gap-4 shrink-0 bg-gray-50/30">
                <button onClick={() => setModalOpen(false)} className="flex-1 py-4 font-black text-gray-500 bg-white border border-gray-100 rounded-2xl active:scale-95 transition-all">{t.cancel}</button>
                <button onClick={handleSave} className="flex-1 py-4 font-black text-white bg-[#483383] rounded-2xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                   <Save size={20} />
                   <span>{t.save}</span>
                </button>
             </div>
