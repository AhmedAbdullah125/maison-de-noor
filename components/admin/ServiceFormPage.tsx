    ...
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-4">
        ...
        <button 
          ...
          className="bg-[#483383] text-white px-6 py-2 rounded-xl font-bold"
        >
          Back to Services
        </button>
      </div>
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fadeIn pb-20 relative">
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button 
            onClick={handleCancel}
            className="px-6 py-3 bg-white border border-gray-100 rounded-2xl font-bold text-gray-400 hover:bg-gray-50 transition-all flex items-center gap-2"
          >
            <X size={18} />
            <span>{t.cancel}</span>
          </button>
          <button 
            onClick={handleSave}
            className="px-8 py-3 bg-[#483383] text-white rounded-2xl font-bold shadow-lg shadow-[#483383]/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2"
          >
            <Save size={18} />
            <span>{t.save}</span>
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t.serviceNameAr}</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t.serviceNameEn}</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t.price} ({t.currency})</label>
                <input 
                  type="text" 
                  className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-[#483383] transition-all" 
                  value={form.price} 
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t.duration}</label>
                <input 
                  type="text" 
                  className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-[#483383] transition-all" 
                  value={form.duration} 
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden transition-all">
             ...
             >
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-lg font-bold text-gray-900">{t.serviceAddons}</h3>
                    <span className="text-[10px] font-black text-white bg-[#483383] px-2 py-0.5 rounded-lg ml-2">
                ...
                <div className="px-8 pb-8 pt-0 animate-fadeIn">
                    ...
                    {globalAddons.length === 0 ? (
                        <div className="py-10 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                            <p className="text-sm text-gray-400 font-bold">No global add-ons defined yet.</p>
                            <button 
                            onClick={(e) => { e.stopPropagation(); navigate('/admin/service-addons'); }}
                            className="text-xs font-black text-[#483383] underline mt-2"
                            >
                                Go to Catalog
                            </button>
                        </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className={`text-sm font-black ${isSelected ? 'text-[#483383]' : 'text-gray-900'}`}>{lang === 'ar' ? addon.titleAr : addon.titleEn}</p>
                                                <p className="text-[10px] text-gray-400 font-bold uppercase">{addon.items.length} Options</p>
                                            </div>
                                        </div>
                                        <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase ${addon.required ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-400'}`}>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
             <div className="flex items-center justify-between border-b border-gray-50 pb-4">
                <div>
                   ...
                   <p className="text-[10px] text-gray-400 mt-1 font-bold">{t.subscriptionsHelper}</p>
                </div>
                <button 
                   onClick={handleAddSubscription}
                   className="bg-[#483383]/10 text-[#483383] px-4 py-2 rounded-xl text-xs font-black flex items-center gap-2 hover:bg-[#483383] hover:text-white transition-all"
                >
                   <Plus size={16} />
             ...
             <div className="space-y-4">
                {form.subscriptions?.length === 0 && (
                   <div className="py-10 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                      <p className="text-sm text-gray-400 font-bold">{t.noContentYet}</p>
                   </div>
                )}
                   ...
                   <div key={sub.id} className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100 relative group transition-all hover:border-gray-200 hover:shadow-sm">
                      <div className="flex justify-between items-start mb-4">
                         <div className="flex-1">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">{t.subscriptionTitle}</label>
                            <input 
                               type="text" 
                               className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-[#483383]"
                               value={sub.title}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         <div>
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">{t.sessionsCount}</label>
                            <input 
                               type="number" 
                               min="2"
                               className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-[#483383]"
                         ...
                         <div>
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">{t.pricePercent}</label>
                            <input 
                               type="number" 
                               min="1"
                               max="999"
                         ...
                         <div>
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">{t.validityDays}</label>
                            <input 
                               type="number" 
                               min="1"
                               className="w-full bg-white border border-gray-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-[#483383]"
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
            <h3 className="text-lg font-bold text-gray-900 border-b border-gray-50 pb-4">Service Media</h3>
            
            {/* Main Image */}
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Main Image (Required)</label>
              {form.image ? (
                  <div className="relative aspect-video rounded-2xl overflow-hidden group border border-gray-200">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                          <label className="cursor-pointer px-4 py-2 bg-white rounded-xl text-xs font-bold hover:bg-gray-100">
                              Change
                              <input type="file" className="hidden" accept="image/*" onChange={handleMainImageUpload} />
                          </label>
                          <button onClick={handleRemoveMainImage} className="px-4 py-2 bg-red-500 text-white rounded-xl text-xs font-bold hover:bg-red-600">Remove</button>
                      </div>
                  </div>
              ) : (
                  <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-[#483383] hover:bg-violet-50 transition-all bg-gray-50">
                      ...
                      <span className="text-sm font-bold text-gray-400 mt-2">Upload Main Image</span>
                      <input type="file" className="hidden" accept="image/*" onChange={handleMainImageUpload} />
                  </label>
              )}
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                  ...
                  <label className="cursor-pointer text-[#483383] text-xs font-bold hover:underline flex items-center gap-1">
              ...
              <div className="grid grid-cols-3 gap-3">
                  ...
                  {(form.images?.length === 0) && (
                     <div className="col-span-3 py-8 text-center border border-dashed border-gray-200 rounded-2xl">
                        ...
                        <p className="text-xs font-bold text-gray-400">No gallery images</p>
                     </div>
                  )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-8 shadow-2xl animate-scaleIn text-center">
            ...
            <div className="flex flex-col gap-3">
              <button 
                onClick={confirmExit}
                className="w-full py-4 bg-red-50 text-red-600 font-bold rounded-2xl active:scale-95 transition-transform"
              >
                {lang === 'ar' ? 'نعم، خروج بدون حفظ' : 'Yes, Leave without Saving'}
              </button>
              <button 
                onClick={() => setShowExitPrompt(false)}
                className="w-full py-4 bg-gray-50 text-gray-700 font-bold rounded-2xl active:scale-95 transition-transform"
              >
